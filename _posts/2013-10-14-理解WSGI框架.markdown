...
在 java web 领域，支持 [servlet API](http://docs.oracle.com/javaee/6/api/?javax/servlet/package-summary.html) 的 java application 都可运行在同样支持 servlet API 的 web server(http server) 上。随着 web 不断发展，python application/framework 也如雨后春笋般涌出，如：Zope, Webware, Skunkweb 等等。但是它们接口存在差异，导致难以跨 web server 运行，所以 python 社区定义了一种如 servlet API 般标准通用的接口\-\-\-WSGI。
...
- 为 [environ](https://www.python.org/dev/peps/pep-0333/#environ-variables) 提供必要的参数，实现一个回调函数 [start_response](https://www.python.org/dev/peps/pep-0333/#id21)，传给 callable object
- 调用 callable object
