...
![cpu l cache](http://7xp2eu.com1.z0.glb.clouddn.com/cpu_cache_l_size.png)
...
对于当前的个人计算机的 CPU，L1 cache 通常为数十 KB，L2 cache 为数百 KB，L3 cache 可达数 MB，但是 TLB 相对较小，一般只有几百个 entry。
...
本节主要介绍内存中的数据在 cache 的存放规则，即对于地址为 A 的数据，它该存放在 cache 的何处？要回答此问题，首先需介绍三种不同的存放规则：
...
从硬件的角度出发，direct mapped cache 设计简单，full associative cache 设计复杂，特别当 cache size 很大时，硬件成本非常之高。但是 direct mapped cache 下数据的存放地址是固定唯一的，所以容易产生碰撞，最终降低 cache 的命中率，影响性能。在成本和性能的权衡下，当前的 CPU 都是 N-way set associative cache，N 通常为 4，8 或 16。

以大小为 32 KB，cache line 的大小为 64 Byte 的某 cache 为例，对于不同存放规则，其硬件设计也不同，下列图片依次展示其原理。

Direct mapped cache

![direct mapping](http://7xp2eu.com1.z0.glb.clouddn.com/cpu_cache_direct_mapping.png)

2-way set associative

![two way associative](http://7xp2eu.com1.z0.glb.clouddn.com/cpu_cache_2_way_mapping.png)

Full associative cache

![full associative](http://7xp2eu.com1.z0.glb.clouddn.com/cpu_cache_full_associative.png)
...
当 STRIDE 为 32768 时，所用时间为 18 s，当 STRIDE 为 1 时，所用时间为 3.5 s。
